...
mydll.F7_FUNCTION.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_int)]
# mydll.F7_FUNCTION.restype = ctypes.POINTER(ctypes.c_int)
# mydll.F7_FUNCTION.restype = None
#
mydll.F8_FUNCTION.argtypes = [ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double,
                              ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_int,
                              ctypes.POINTER(ctypes.c_int)]

mydll.get_roads_by_id.argtypes = [ctypes.c_int, np.ctypeslib.ndpointer(dtype=np.double, shape=(150, 2))]

mydll.get_roads_by_id.restype = None

# 调用F7_FUNCTION函数
k = 10
x = 500
res_id = (ctypes.c_int * 72183)()  # 创建一个包含2000个整数的数组
# mydll.F7_FUNCTION(k, x, res_id)
# mydll.F8_FUNCTION(116.32940 , 39.86751 , 116.40910 , 39.93210, \
#                   116.32940 , 39.86751 , 116.40910 , 39.93210, k, res_id)
# res_id_ptr = mydll.F7_FUNCTION(k, x)
# res_id = ctypes.cast(res_id_ptr, ctypes.POINTER(ctypes.c_int))
# 打印结果

mydll.F8_FUNCTION(116.55640, 40.00151, 116.73210, 39.63210, \
                  116.25640, 39.93200, 116.53210, 39.73110, k, res_id)
# dll.mydll.F8_FUNCTION(ctypes.c_double(lon_1), ctypes.c_double(lat_1), ctypes.c_double(lon_2), ctypes.c_double(lat_2), \
#     ctypes.c_double(lon_1), ctypes.c_double(lat_1), ctypes.c_double(lon_2), ctypes.c_double(lat_2), k, res_id)
# 116.35640, 39.89200, 116.43210, 39.88110
...
def get_all_roads_by_id(res_id):  # 频度由res_id来体现。
    ...
    for i in range(72183):
        ...
        if (res_id[i] != -1):
            # print(i + 1, res_id[i])
            res = np.zeros((150, 2), dtype=np.double)
...
# mydll.get_roads_by_id(id, res)

# 打印结果
# print("get_roads_by_id结果：")
# for i in range(100):
#     if res[i][0] == 0:
#         break;
#     print(f"经度: {res[i][0]}, 纬度: {res[i][1]}")
...
with open('new.html', 'w', encoding='utf-8') as k:
    ...
    all_roads = get_all_roads_by_id(res_id)
    print("3333333333333")
